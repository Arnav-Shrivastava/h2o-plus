...
export default function HistoryScreen() {
  ...
  // Create an array mapping from history to the graph data structure
  const weekData = history.map(h => ({
    day: h.day,
    pct: Math.min(1, h.total / Math.max(1, h.goal)), // Max 1.0 (100%)
    goal: h.met
  }));
  
  // Reverse history so most recent day (today) is at the top of the list
  const dailyLogs = [...history].reverse();

  // Compute consistency score and days met
  const daysMet = history.filter(h => h.met).length;
  const totalDays = history.length;
  const consistencyScore = totalDays > 0 ? Math.round((daysMet / totalDays) * 100) : 0;
  return (
        ...
        <View className="flex-row gap-3 mb-5">
            ...
            <View style={{ flexDirection: "row", alignItems: "flex-end", height: 100, gap: 6 }}>
              {weekData.map((d, i) => (
                <View key={i} style={{ flex: 1, alignItems: "center", gap: 6, height: "100%" }}>
                  ...
                  <Text style={{ fontFamily: "Manrope", fontSize: 10, color: d.goal ? "#0058bf" : "#414755", fontWeight: d.goal ? "bold" : "normal" }}>{d.day}</Text>
                </View>
              ))}
            </View>
          </View>
          ...
          <View style={{ gap: 10, width: 110 }}>
            ...
            <View style={{ flex: 1, backgroundColor: "#56f5f8", borderRadius: 20, padding: 16, justifyContent: "space-between" }}>
              ...
              <View>
                <Text style={{ fontFamily: "PlusJakartaSans", fontWeight: "800", fontSize: 28, color: "#006e70" }}>{consistencyScore}%</Text>
            ...
            <View style={{ flex: 1, backgroundColor: "#686dbe", borderRadius: 20, padding: 16, justifyContent: "space-between" }}>
              ...
              <View>
                <Text style={{ fontFamily: "PlusJakartaSans", fontWeight: "800", fontSize: 24, color: "#fffbff" }}>{daysMet}/{totalDays}</Text>
              ...
              <View className="flex-row items-center gap-3">
                ...
                <View>
                  <Text className="font-bold text-sm text-on-surface" style={{ fontFamily: "Manrope" }}>
                    {log.fullDay === "Mon" ? "Monday" :
                     log.fullDay === "Tue" ? "Tuesday" :
                     log.fullDay === "Wed" ? "Wednesday" :
                     log.fullDay === "Thu" ? "Thursday" :
                     log.fullDay === "Fri" ? "Friday" :
                     log.fullDay === "Sat" ? "Saturday" : "Sunday"}
                  </Text>
